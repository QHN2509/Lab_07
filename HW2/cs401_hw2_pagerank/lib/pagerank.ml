...
let allPages graph =
  List.fold_left
      (fun pages (src, dst) -> PageSet.add dst (PageSet.add src pages))
      PageSet.empty graph
...
*)
let numPages graph =
  PageSet.cardinal (allPages graph)
...
let initPageRank graph =
    let pages = allPages graph in
    let n = float_of_int (PageSet.cardinal pages) in
    PageSet.fold
      (fun page pr -> PageRank.add page (1.0/.n) pr)
  ...
  PageSet.fold
    (fun page new_pr ->
      ...
      let s =
        PageSet.fold
          (fun backlinks acc ->
            let backlinks_rank = PageRank.find backlinks pr in
            let outbound = float_of_int (numLinks graph backlinks) in
            acc +. (backlinks_rank /. outbound))
          backlinks 0.0
      in
      PageRank.add page (base +. (d *. s)) new_pr)
    pages PageRank.empty
...
let iterPageRank pr d graph delta =
    let rec loop curr_pr =
      let next_pr = stepPageRank curr_pr d graph in
      let max_change =
        PageRank.fold
          (fun page rank acc ->
            let old_rank = PageRank.find page curr_pr in
            max acc (abs_float (rank -. old_rank)))
          next_pr 0.0
      in
      if max_change < delta then next_pr else loop next_pr
    in
    loop pr
